...
public class GeneralConverters {
	...
	public static BigInteger termToRoundInteger(Term value, ChoisePoint iX, boolean acceptDateAndTime) throws TermIsNotAnInteger {
		...
		try {
			...
			try {
				result= doubleToBigInteger(value.getRealValue(iX));
	...
	///////////////////////////////////////////////////////////////
	//
	public static byte[] argumentToBinary(Term value, ChoisePoint iX) {
		try {
			return value.getBinaryValue(iX);
		} catch (TermIsNotABinary e) {
			throw new WrongArgumentIsNotABinary(value);
		}
	}
	//
	///////////////////////////////////////////////////////////////
	...
	public static String bytesToString(byte[] byteArray) {
		...
		if (numberOfBytes % 2 == 0) {
			numberOfCharacters= numberOfBytes / 2;
		} else {
			numberOfCharacters= numberOfBytes / 2 + 1;
		};
		char[] codes= new char[numberOfCharacters];
		for (int n=0; n < numberOfCharacters; n++) {
			int index1= n * 2;
			int index2= index1 + 1;
			int firstByte= byteArray[index1] & 0xFF;
			int secondByte;
			if (index2 < numberOfBytes) {
				secondByte= byteArray[index2] & 0xFF;
			} else {
				secondByte= 0;
			};
			int characterCode= firstByte << 8 | secondByte;
			codes[n]= (char)characterCode;
		};
		return new String(codes);
	}
	//
	...
	public static double[][] argumentToMatrix(Term value, ChoisePoint iX) {
		Term[] termArray= listToArray(value,iX);
		int numberOfRows= termArray.length;
		double[][] rows= new double[numberOfRows][];
		int numberOfColumns= -1;
		for (int n=0; n < numberOfRows; n++) {
			Term[] columnArray= listToArray(termArray[n],iX);
			...
			for (int m=0; m < numberOfColumns; m++) {
				rows[n][m]= argumentToReal(columnArray[m],iX);
			}
		};
		return rows;
	}
